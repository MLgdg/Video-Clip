...
class UniterVideoEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.voxel_to_patch = torch.nn.Conv3d(in_channels=3, out_channels=config.hidden_size //6 ,\
        ...
        self.patch_to_Tr = nn.Linear(config.hidden_size // 6, config.hidden_size)
    ...
    def forward(self, inputs, video_position_ids, video_attn_masks, token_type_ids=None):
        ...
        embeddings = self.patch_to_Tr(embeddings)
        ...
        temporal_position_ids = video_position_ids[:, :, 0]
        vertical_position_ids = video_position_ids[:, :, 1]
        horizontal_position_ids = video_position_ids[:, :, 2]
