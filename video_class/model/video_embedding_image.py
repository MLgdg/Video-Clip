...
class UniterVideoEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        patch_dim = 3 * config.spatial_patch_size * config.spatial_patch_size
        self.to_patch_embedding = nn.Sequential(
            Rearrange('b t c (h p1) (w p2) -> b t (h w) (p1 p2 c)', p1 = config.spatial_patch_size, p2 = config.spatial_patch_size),
            nn.Linear(patch_dim, 48),
        )
        ...
        self.temporal_embeddings = nn.Embedding(config.max_image,
    ...
    def forward(self, inputs, video_position_ids, video_attn_masks, token_type_ids=None):
        embeddings = self.to_patch_embedding(inputs)
        b, t, x, y = embeddings.shape
        ...
        temporal_embeddings = self.temporal_embeddings(video_position_ids)
