...
class UniterAudioEmbeddings(nn.Module):
    """
    考虑极限情况下的特征维度
    embedding最大2000 每一帧是512采样点，最大支持45秒
    """
    def __init__(self, config):
        super().__init__()
        self.config = config
        #self.max_num_patches = int(config.patch_sampling_rate * max_positional_buckets)
        self.wave_to_patch = torch.nn.Conv1d(in_channels=1, out_channels=config.hidden_size,\
                                             kernel_size=config.audio_temporal_patch_size,
                                             stride = config.audio_temporal_patch_size)
    ...
    def _random_patch_selection(self, inputs, audio_attn_masks):
        ...
        if training:
            ...
            temporal_idx = torch.LongTensor(sorted(random.sample(range(seq_len),\
                    int(seq_len * self.config.patch_sampling_rate)))).to(inputs.device)
    ...
    def forward(self, inputs, audio_position_ids, audio_attn_masks, token_type_ids=None):
        ...
        embeddings, audio_attn_masks = self._random_patch_selection(embeddings, audio_attn_masks)
