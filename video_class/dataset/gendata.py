...
class UVATDataset(Dataset):
    ...
    def get_audio_input(self, info):
        if 'video_audio_path' not in info:
            return torch.zeros(0, dtype=torch.float), 0
        _, wav = wavfile.read(os.path.join(root_path, info['video_audio_path']))
        len_audio = (min(config.max_audio, len(wav))) // 512 * 512
        wav = np.array(wav)
        return torch.tensor(wav[:len_audio]), len_audio
...
def pad_tensors_audio(tensors, lens=None, pad=0):
    ...
    #h, w = tensors[0].shape[2:] #tensors[0].size(-1)
    dtype = tensors[0].dtype
    output = torch.zeros(bs, max_len, dtype=torch.float32)
