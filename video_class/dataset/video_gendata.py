...
class UVATDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        image_attn_masks = torch.ones(image_number, dtype=torch.long)
    ...
    def get_img_input(self, info):
        ...
        num_bb = min(len(img_names), self.max_lenth)
        #num_bb = len(img_names)
...
def vqa_collate(inputs):
    ...
    if len(img_feat) !=0:
        ...
        video_attn_masks = torch.zeros([bs, tmp_image_mask_shape], dtype=torch.long)
        for i, t in enumerate(image_attn_masks):
            video_attn_masks.data[i, :len(t)] = t.data
        video_position_ids =torch.arange(0, img_feat.size(1), dtype=torch.long
                                    ).unsqueeze(0)# _build_vid_pos_ids(tmp_image_mask_shape // temporal_patch_size, w // spatial_patch_size, w // spatial_patch_size)
    else:
