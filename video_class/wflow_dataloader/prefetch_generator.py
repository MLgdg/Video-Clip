...
class BackgroundGenerator(threading.Thread):
    def __init__(self, generator, max_prefetch=1, data_processer=None, data_processer_batch_size=1):
        """
        ...
        #self.executor = ThreadPoolExecutor(max_workers=max_prefetch)
        from concurrent.futures import ProcessPoolExecutor
        self.executor = ProcessPoolExecutor(max_workers=max_prefetch)
    ...
    def run(self):
        ...
        for item in self.generator:
            ...
            #print(e - s, "get")
            items.append(item)
            if len(items) >= self.data_processer_batch_size:
                items = self.executor.submit(self.data_processer, items)
